...
public class FadeInObject
{
    ...
    private void buildJson ()
    {
        ...
        taskJson.put(AnimationKey.Component.FadeIn.GRAPHICS, taskToJsonList(graphicsTasks));
        taskJson.put(AnimationKey.Component.FadeIn.UI, taskToJsonList(uiTasks));
    ...
    {
        // 解析 graphics 数组
        JsonEntity graphicsNode = taskJson.getJsonEntityByKey(AnimationKey.Component.FadeIn.GRAPHICS);
        if (graphicsNode.isList())
        {
            for (int i = 0; i < graphicsNode.size(); i++)
            {
                ...
                {
                    LogUtils.error(FadeInObject.class, "parseTasks graphics 任务解析无效 (index): " + i + " (json): " + graphicsNode.getJsonEntityByIndex(i));
                }
            }
        }

        // 解析 ui 数组
        JsonEntity uiNode = taskJson.getJsonEntityByKey(AnimationKey.Component.FadeIn.UI);
        if (uiNode.isList())
        {
            for (int i = 0; i < uiNode.size(); i++)
            {
                ...
                {
                    LogUtils.error(FadeInObject.class, "parseTasks ui 任务解析无效 (index): " + i + " (json): " + uiNode.getJsonEntityByIndex(i));
                }
            }
        }
    }
     ...
     */
    public FadeInObject (float duration, List<AnimationTask> graphicsTasks, List<AnimationTask> uiTasks)
    {
