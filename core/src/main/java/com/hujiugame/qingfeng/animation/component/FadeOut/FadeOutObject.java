...
public class FadeOutObject
{
    ...
    public FadeOutObject(float duration, Set<AnimationTask> synchronizationTasks, List<AnimationTask> scheduleTasks)
    {
        ...
        if (synchronizationTasks != null)
        {
            for (AnimationTask task : synchronizationTasks)
            {
                ...
                {
                    LogUtils.debug(FadeOutObject.class, "字段构造 未知任务目标类型，忽略: " + task);
                }
            }
        }
    ...
    public FadeOutObject(JsonEntity json)
    {
        ...
        if (json != null && json.isMap() && json.containsKey(AnimationKey.Component.FadeOut.DURATION))
        {
            ...
            this.scheduleTasks.addAll(parseScheduleTasks(taskJson.getJsonEntityByKey(AnimationKey.Component.FadeOut.SCHEDULE)));
