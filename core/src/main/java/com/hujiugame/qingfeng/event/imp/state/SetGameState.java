...
/**
 * 设置游戏状态事件（空壳事件），用于直接替换当前游戏状态。
 * <p>
 * 仅记录切换意图，不携带动画组件；淡入淡出动画由过渡链按页获取
 * （淡出取当前页动画，淡入取切换后目标页动画）。
 */
public class SetGameState implements Event
{
    private final EventAction eventAction = EventAction.SET_GAME_STATE;
     ...
     * @param inState 进入状态
     */
    public SetGameState (GameState outState, GameState inState)
    {
