...
>
> **2026-08-15/17 演进落地：** 方案演进出"空壳事件 + 执行事件双事件模型"（见 `temp/CLAUDE_MEMORY.md` 2026-08-17 条目）与控件级 smooth_move 动画（TransitionManager 状态机 + 预偏移 + 时间戳计时），已实现落地。本文档作为源头方案部分过时，控件级动画实际配置结构见各页 `config.json` 的 `animation` 节点。
>
> **待解决问题（2026-08-18）：** 淡出结束 → 淡入开始之间存在卡顿，卡顿时段被计入淡入计时，导致淡入前段"有计时但控件无法操作/未动"（表现为淡入显示不完整、最后一两个动画只走了半程）。根因：切页瞬间新页 `onInit` 建控件耗时 + 帧 delta 突变，虽已用时间戳计时缓解，但切页卡顿本身仍被包含进淡入计时窗口，需进一步消除（方向：切换完成并建好控件后再启动淡入计时，或预载目标页布局）。
>
