...
## 2026-08-16 — 12 个新语言翻译包 + 首次运行按设备语言改写默认语言 + 设备语言自动检测与 15 语言扩展 + 键前缀/标签前缀常量注释补充动态段占位标注 + AnimationManager 一级服务化 + 显示配置下沉 UserConfigManager + 全屏切换职责迁移 + 崩溃处理职责下沉 CrashUtils + RenderPipeline 方法重命名区分注册与更新 + 全屏切换调用空值防御

**新增(资产)：language_config 注册 15 语言与 12 个新语言翻译包**

### 资产

- **language_config.json 注册 15 语言** — 全部语言名称统一为 `XXX语言·地区` 格式（如 `Français·France`），新增 zh_HK/en_GB/es_ES/es_MX/fr_FR/fr_CA/pt_BR/pt_PT/ru_RU/ja_JP/ko_KR/de_DE 12 个语言条目（commit ）
- **12 个新语言目录翻译包** — 每个目录含 language.json（name/version/default/blocks 声明 requirement.json）与 requirement.json（message_box/menu.main/menu.list/menu.load/config.basic 全套 UI 翻译），键结构与 en_US 完全一致（commit ）
