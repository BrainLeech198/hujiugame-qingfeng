...
**文档(发布)：建立版本发布记录体系 + CHANGELOG 条目绑定提交哈希**

### 文档

- **新增 `develop/PUBLISH.md` 版本发布记录** — 记录每次发布版本（beta/正式版）的版本号、版本类型、发布时间戳（= 最后一次更新版本下载地址 `docs/data/versions.json` 的 git 提交时间）、该版本最后一次更新 commit、面向玩家的 bug 修复/新增功能摘要；并写入 v1.0.0-beta-26w33a 发布记录（发布时间 2026-08-13，最后一次更新 `adf2635`）
- **CHANGELOG 头部新增规则 9/10** — 规则 9：每次发布新版本须在 PUBLISH.md 记录发布信息与时间戳；规则 10：同一天更新归组到同一日期标题、条目绑定引入它的 git 提交短哈希
- **回填 CHANGELOG 全部条目 git 提交短哈希** — 138 个提交块按日期归组为 49 个日期（一天一个 `## <日期>` 标题），673 条条目行尾标注引入它的 commit 短哈希
