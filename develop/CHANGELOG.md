...
**修复(官网)：子路径部署下运行时资源绝对路径 404（commit ）**

### 修复

- **资源路径相对化** — GitHub Pages 子路径部署下 `/data/...`、`/resource/...` 绝对路径解析到域名根导致 404（i18n key 原文、下载数据缺失、图片不显示）；全部改为相对路径，模板静态 img 改 `:src` 动态绑定避免 Vite 模块解析（commit ）
