...
## 2026-08-16 — 设备语言自动检测与 15 语言扩展 + 键前缀/标签前缀常量注释补充动态段占位标注 + AnimationManager 一级服务化 + 显示配置下沉 UserConfigManager + 全屏切换职责迁移 + 崩溃处理职责下沉 CrashUtils + RenderPipeline 方法重命名区分注册与更新 + 全屏切换调用空值防御

**新增(类型)：DeviceLanguage 设备语言检测枚举与 15 语言路径/显示名常量**

### 类型

- **FileName 语言路径常量全覆盖 15 语言** — 新增 12 个语言路径常量（zh_HK/en_GB/es_ES/es_MX/fr_FR/fr_CA/pt_BR/pt_PT/ru_RU/ja_JP/ko_KR/de_DE），zh_CN/zh_TW/en_US 命名化为常量，DEFAULT_LANGUAGE_PATH 由 zh_CN 改为 en_US（commit ）
- **Name 语言显示名常量全覆盖 15 语言** — 新增 12 个语言显示名常量（`XXX语言·地区` 格式，如 `Français·France`），DEFAULT_LANGUAGE_NAME 改为 English·United States（commit ）
- **DeviceLanguage 设备语言检测枚举** — 15 个语言枚举（语言_地区 大写目录名），`fromLocale` 按语言+地区精确匹配、未命中回退该语言默认地区、不支持语言回退英文，`detectDefault` 基于 `Locale.getDefault()` 四平台通用（commit ）
