...
## 2026-08-15 — 官网下载区优化（恢复提取码 / 首页卡片化 / 更新时间）+ 26w33a mac Intel 提取码文案补删 + GameState 扁平化重构

**重构(状态系统)：GameState 扁平化，消除主次分层**

### 重构

- **GameState 扁平枚举化** — 原 `GameState`（int 主状态）+ `GameSubState`（int 子状态）+ `StateStructure`（双 int 数据类）+ `GameStatePageInfo`（两层 Map）整合为单一扁平枚举 `type/game/GameState`，枚举值即完整可流转状态，携带布局目录名/是否需要配置/是否游戏内/中文显示名元数据；主状态本身无独立页面，真正可流转 9 个叶子状态；删除全项目零引用的 SAVE/EDIT 死代码（commit <hash>）
- **状态流转调用点全面改造** — `SceneStack` 改 `Stack<GameState>`、`GameRenderRegistry` 改 `Map<GameState, Supplier>` 枚举键、`PushGameState`/`SetGameState` 事件携带枚举、`GameStateDataContainer` 持有枚举、渲染机注册改用枚举、页面类事件参数同步更新（commit <hash>）
- **修复 CONFIG_DISPLAY 休眠缺陷** — `InstanceContent` 补注册 `ConfigDisplay` 渲染机（此前无注册，推到该状态会崩溃）（commit <hash>）
- **CODING_STYLE 同步** — 移除已删除的 `StateStructure`/`getGameStateName` 示例引用，改用现存类（commit <hash>）
