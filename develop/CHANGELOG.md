...
## 2026-08-13 — 修复(mac)：原生弹窗改用 osascript 避免 AWT 死锁

### 修复

- **macOS 原生弹窗改用 osascript（非 AWT）** — 新增 `lwjgl3/imp/MacOsAppleScriptDialog.java`（AppleScript `display dialog` 实现），`Lwjgl3Launcher` macOS 分支注入替换 Swing `JOptionPane`（含崩溃弹窗），消除同类 AWT 与 LWJGL3 主线程死锁隐患；其余平台保持 Swing 实现不变

---

## 2026-08-13 — 修复(mac)：点击路径打开资源管理器死锁

### 修复

- **macOS 点击路径标签打开资源管理器不再死锁** — `lwjgl3/imp/DesktopExplorerOpener.java` macOS 上完全绕开 `java.awt.Desktop`（AWT 与 LWJGL3 抢占主线程会死锁卡死游戏），改走 CLI `open` 命令；进程启动移入后台守护线程消费输出，不再阻塞 GL 线程（Windows/Linux 保持 AWT 优先 + CLI 回退）
...
- **下载区五个平台链接换新** — `docs/data/versions.json` Windows、Android、Linux、mac Apple 芯片版、mac Intel 版五处蓝奏云下载链接全部替换为新地址（提取码 1698 不变）

---

## 2026-08-13 — 网站(版权)：官网维修步骤示意图换为游戏运行截图

### 网站

- **官网维修步骤示意图换为游戏运行截图** — `docs/resource/image/repair1.png`、`repair2.png` 由第三方素材替换为游戏运行截图（步骤一维修标识 / 步骤二确定退出），`docs/THIRDPARTY_LICENSES.html` 登记「原创」不变

---

## 2026-08-13 — 资产(版权)：主菜单/启动背景图换为自制素材

### 资产

- **主菜单/启动背景图换为自制素材** — `menu.background.png`、`app_init.png` 由豆包AI第三方素材替换为自制素材（AI 生成 + 手工二次设计），`assets/THIRDPARTY_LICENSES.md` 及官网 `docs/PROJECT_THIRDPARTY.html` 相应条目由「1.1 AI 生成」移入「三、原创素材」

---

## 2026-08-13 — 官网(修复)：许可证页 CC 联系链接超链接化

### 网站

- **许可证页 CC 联系链接超链接化** — `docs/LICENSE.html` 简/繁/EN 三个 tab 末尾的 Creative Commons 联系入口由纯文本改为指向 https://creativecommons.org/ 的超链接（新窗口打开），与 CC 官方模板一致

---

## 2026-08-13 — 文档(清理)：删除已实施完成的官网美化设计文档

### 文档

- **移除已落地方案的陈旧文档** — 官网玻璃拟态美化方案已全量实施，删除 `develop/plans/2026-08-13-website-beautify.md` 设计文档（原未登记 DOCUMENTATION_INDEX）

---

## 2026-08-13 — 文档(计划)：mac zip 交付设计记录与实施计划

### 文档

- **mac zip 交付设计记录** — `develop/plans/2026-08-13-mac-zip-delivery.md` 记录 mac 交付物从「裸 .app zip + .command」调整为「zip 内嵌自解压 .command + 中文 symlink」的设计（蓝奏云扩展名白名单 / 无 macOS 环境 / 命名定案）
- **mac zip 交付实施计划** — `develop/plans/2026-08-13-mac-zip-delivery-plan.md` 逐任务拆解 `build_package_mac.py` 改造与一次性补救脚本的实施步骤
...
- **beta 快照版本方案更新** — 明确更新检测在整型相同时按日期码（快照码）细分新旧；官网下载区两阶段弹窗（先选来源再列描述+链接）与 `versions.json` 数据结构映射说明

---

## 2026-08-13 — 官网(修复)：历史版本页补 mac 下载按钮

### 网站

- **历史页补 mac 下载按钮** — `history.js` 版本卡下载行补上 Mac 下载按钮（`download-mac.png` + `data-platform="mac"`），`history.css` 下载网格由 3 列扩为 4 列并在 700px 断点降为 2 列；mac 的 Apple/Intel 双来源由弹窗来源选择展示

---

## 2026-08-13 — 官网(beta)：下载区接入蓝奏云真实链接

### 网站

- **官网下载区接入真实链接** — `versions.json` 下载区替换示例链接为蓝奏云真实地址（含提取码 1698），Windows/Android/Linux/mac（含 Intel 双来源）各一处，描述补充安装指引；顶层新增 `newest_version_snapshot` 字段供更新检测按快照码判断

---

## 2026-08-13 — 版本(beta)：测试版更新弹窗 + 版本切 beta 快照

### 功能

- **测试版更新弹窗** — 检测到测试版更新时，弹窗标题/正文改用「发现测试版更新」并提示可能不稳定（`MenuMain` 按 `isNewestVersionBeta()` 切换文案 key，requirement.json 三语言新增 `update_detected_beta.*`）
- **版本切 beta 快照** — `app_version.json` 版本类型由 release 切为 beta、快照码置 `26w33a`

---

## 2026-08-13 — 版本(beta)：更新检测整型相同时按快照码细分新旧

### 功能

- **beta 快照码更新判断** — 更新检测主判断整型版本号相等时，双方均为测试版则再按快照码（`YYwWWa` 定宽字典序即时间序）细分新旧，快照码升级判定为可更新；`VersionKey` 新增 `NEWEST_VERSION_SNAPSHOT`、`VersionType` 新增 `UNKNOWN`，`UpdateChecker` 记录远程版本类型并暴露 `isNewestVersionBeta()`

---

## 2026-08-13 — 官网：深色主题配色微调（卡片暖墨分层 + 页脚下划线生长修复）

### 网站

- **卡片暖墨分层** — 深青背景下的卡片由同色系青色改为中性暖墨灰（`--card-bg` 等色板变量收敛到 common.css），hover/下载标签/hero 分层暖墨，青色只作强调色（边框/按钮/链接/光晕），背景青与卡片墨形成冷暖对比
- **页脚下划线生长修复** — 全站页脚链接下划线由 2px 加粗到 3px、过渡由 0.3s 放慢到 0.45s（index/history/community/license 四页同步），生长过程在深色背景上清晰可见

---

## 2026-08-13 — 官网：许可证页深色化

### 网站

- **许可证页深色化** — 三页许可证页挂载 common.css，容器/表格/引用块/代码/tab 按钮全量深色，正文链接与页脚下划线统一青色

---

## 2026-08-13 — 官网：社区页深色化

### 网站

- **社区页深色化** — 社区页挂载 common.css（共享深色色板/氛围层/emoji 动效），讨论卡/返回钮/页脚全量深色，讨论标题青白渐变字

---

## 2026-08-13 — 官网：历史页深色化

### 网站

- **历史页深色化** — 版本卡/返回钮/分页/下载按钮/页脚全量深色，版本标题青白渐变字，hover 加青色辉光；「最新」标签改暖金点缀，与首页形成同套分级
...
- **首页深色化** — hero/提示卡/修复面板/卡片/下载区/更新日志/社区/页脚/吸顶导航全量深色，标题改青白渐变字 + 光晕，按钮改青色渐变 + 辉光，卡片 hover 上浮加青色氛围光；hero 顶部青色高光分层、logo 投影改青色辉光
