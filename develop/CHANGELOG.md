...
## 2026-09-02 — 官网迁移至 Vue 3 + Vite 构建 + 版权许可增强 + 引擎分离文档定稿 + 官网子路径资源 404 修复
...
**重构(官网)：页面组件化 Vue 迁移（commit 5cbf5c4）**

### 重构

- **SPA 组件化改造** — 静态多页面重构为 Vue 单页应用：TopNav / SiteFooter / SkipLink / DownloadModal / Lightbox 公共组件 + Home / Help / History / Community / Copyright / License / Thirdparty 七个页面视图（commit 5cbf5c4）

### 网站

- **CC BY-NC 4.0 官方链接** — license 页正文补充 Creative Commons 官方许可链接（by-nc/4.0），明确项目授权条款（commit 5cbf5c4）
- **CC 官方徽章按钮** — 版权页标题栏新增 licensebuttons.net 官方 88×31 徽章，悬停增强 + 移动端缩放适配（commit 5cbf5c4）
- **副标题接入 i18n** — 首页打字机文案改用语言文件 key，切换语言自动以新语言重播（commit 5cbf5c4）
- **减动效淡入** — `prefers-reduced-motion: reduce` 下跳过逐字动画、改为整句淡入，避免文字生硬闪现（commit 5cbf5c4）

### 资产

- **10 语言文件补充** — locales 新增 `close` key；下载图标 / logo / 修复步骤图迁入 `public/`（commit 5cbf5c4）
...
**文档(plans)：引擎分离实施计划 + 一审审查报告 + 方案选项（commit a2b5783）**

### 文档

- **6 阶段实施计划** — 按两轮审查决策修正 Phase 3a（SM 双 API + 帧尾 flush）、3b（Application 主循环 + Init 进栈 + 输入更新）、5（ActiveContext 动态取 AM）；每阶段含痛点/改动/收益/风险表/文件清单/检查清单（commit a2b5783）
- **一审代码级审查报告** — 逐帧推演验证方案假设真实性：3 P0 + 12 P1/P2 问题清单；修正过时认知（TransitionManager 已空壳、场景切换双路径）（commit a2b5783）
- **方案选项与决策记录** — 15 个问题各 2-3 候选方案 + 权衡 + 用户逐项决策记录（commit a2b5783）

---

**文档(plans)：引擎分离进度快照 + 二次审查报告（commit 672499c）**

### 文档

- **跨会话进度断点** — 当前状态 + 15 项决策汇总 + 已完成/待办清单 + 提交范围红线（只提交 plans 文档，不碰官网改动）（commit 672499c）
- **二次审查报告** — 功能效果与现状一致性二次代码级审查：7 P0 + 5 P1 + 4 P2；12 项决策已确认并回写设计文档与实施计划（commit 672499c）

---

**修复(官网)：子路径部署下运行时资源绝对路径 404（commit 0596a72）**

### 修复

- **资源路径相对化** — GitHub Pages 子路径部署下 `/data/...`、`/resource/...` 绝对路径解析到域名根导致 404（i18n key 原文、下载数据缺失、图片不显示）；全部改为相对路径，模板静态 img 改 `:src` 动态绑定避免 Vite 模块解析（commit 0596a72）
