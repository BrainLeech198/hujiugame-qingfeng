...
> **文档定位**：脚本引擎的完整规范，涵盖 Block JSON 指令语言和引擎内部编码约定。
>
> **文档结构**：
> - 第一部分（脚本指令语言规范）按 `总览 → 脚本结构 → 指令集 → 值系统 → 变量系统 → 控制流 → 执行模型` 顺序编排
> - 第二部分（引擎内部编码规范）按 `分层架构 → 命名约定 → Action-Param 模式 → Parser 约定 → 枚举定义` 顺序编排
> - 每条指令/值命令独立小节，包含：JSON 结构、字段说明、执行语义、示例
>
> **更新规范**：
> 1. 【必须】更新 `develop/CHANGELOG.md` 记录本次变更
> 2. 【必须】新增/修改指令集或值系统时，同步更新第一部分的对应章节
> 3. 【必须】修改引擎编码约定时，同步更新第二部分的对应章节
> 4. 【如果】新增 JSON 配置格式 → 同步更新 `develop/JSON_STANDARD.md`
